...
###Matching estimation and formation period to get the total number of stocks as the final stocks for the data requirement
 ...
 Matched_stocks22<-inner_join(stocks_count22_estimation,stocks_count22_formation,by = join_by(permno)) |> 
   select(permno)
                ...
                #####Part 2: CAPM and other estimation####
install.packages("slider")##rolling estimation
install.packages("furrr")##parallel processing
library(slider)
library(furrr)
##organised data: combining both dataset##
...
##create a time-series of Beta:Time-series Beta## Using Slider
...
##Rolling Window Estimation of β
...
#Let's use a subset, “filter()” firms between “permno” 10000 and 10100
capm_100 <- capm_data |> filter(permno >= '10000' & permno <= '10100')#iterate using “group by()”

##estimation beta s=using the roll capm estimation##
beta <- capm_data |>
  group_by(permno) |>
...
beta_db <- tbl(MAF900_data, "beta")
beta <- beta_db |> 
  collect()

##sorting portfolios by beta##
beta_lag <- beta |>
  mutate(month = month %m+% months(1)) |>
  mutate(beta_monthly= beta)|>
  select(permno, month, beta_monthly) |> 
  drop_na()

data_for_sorts <- inner_join(capm_data, beta_lag, by = c("permno", "month"))

beta_portfolios <- data_for_sorts |>
  group_by(month) |>
  mutate(
    breakpoint = median(beta_monthly),
    portfolio = case_when(
      beta_monthly <= breakpoint ~ "low",
      beta_monthly > breakpoint ~ "high"
    )
  ) |>
  group_by(month, portfolio) |>
  summarize(ret = weighted.mean(ret_excess, mkt_excess), 
            .groups = "drop")
